...
BFloat16& BFloat16::operator = (s16 to_copy)
{
	set_data(to_copy);
	...
	set_sign(get_bits_with_range(data(), (width - 1), (width - 1)));

	u16 absolute_data = abs(static_cast<s16>(data()));
	//s16 absolute_data = abs(data());
...
BFloat16 BFloat16::operator + (const BFloat16& other) const
{
	...
	const size_t d = abs(static_cast<s32>(a.enc_exp()
		- b.enc_exp()));
	...
	auto shift_significand 
		= [&](decltype(a_significand)& some_significand) -> void
	{
		...
		{
			const bool sticky = get_bits_with_range(some_significand,
	...
	if (effective_add)
	{
		...
		if (get_bits_with_range(ret_significand, 8 + num_buffer_bits,
			8 + num_buffer_bits))
		{
			ret_significand >>= 1;
	...
	else // if (!effective_add)
	{
		...
		if (get_bits_with_range(ret_significand, 15, 15))
		{
		...
		if (ret_significand)
		{
			//while (!get_bits_with_range(ret_significand,
			...
			const size_t real_num_leading_zeros 
				...
				- ((sizeof(s8) * 8) + num_buffer_bits));

			if ((static_cast<s32>(ret.enc_exp())
				- static_cast<s32>(real_num_leading_zeros)) <= 0)
			{
				ret.set_enc_exp(0);
			}
			else
...
BFloat16 BFloat16::operator * (const BFloat16& other) const
{
	...
	s32 ret_enc_exp = a.enc_exp() + b.enc_exp() - bias;
	...
	// If necessary, saturate to highest valid enc_mantissa and exponent
	if (ret_enc_exp >= static_cast<s32>(max_enc_exp))
	{
...
BFloat16 BFloat16::operator / (const BFloat16& other) const
{
	...
	s32 ret_enc_exp = a.enc_exp() - b.enc_exp()
		+ bias;
	...
	else // if (ret_significand)
	{
		...
		// does.
		if (get_bits_with_range(ret_significand, 8, 8))
		{
			ret_significand >>= 1;
		}
		else
...
BFloat16::operator float() const
{
	...
	set_bits_with_range(temp, static_cast<u32>(sign()), 
...
BFloat16::operator s16() const
{
	u32 ret = 0;
	
	ret = frac();

	const s32 curr_exp = exponent();
	const s32 abs_curr_exp = abs(curr_exp);

	static constexpr s32 max_shift_amount = (sizeof(s16) * 8) - 1;
...
std::ostream& operator << (std::ostream& os, const BFloat16& to_print)
{
	return osprintout(os, to_print.sign(), ", ",
		//std::hex,
		(s16)(u8)to_print.exponent(), ", ",
		(s16)(u8)to_print.enc_exp(), ", ",
		//std::hex,
		"\t\t", to_print.frac(), ", ",
		//"\t\t", to_print.frac()
