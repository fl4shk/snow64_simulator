#ifndef src__slash__bfloat16_class_hpp
#define src__slash__bfloat16_class_hpp
...
public:		// constants
	...
	static constexpr s32 bias = 127;
	static constexpr s32 amount_to_add_for_modded_bias = 7;
	static constexpr s32 modded_bias = bias
...
public:		// functions
	...
	static BFloat16 fma(const BFloat16& left, const BFloat16& mul_a,
		const BFloat16& mul_b);
	...
	static inline BFloat16 create_from_int(CreateFromType to_cast)
	{
		...
		{
			//absolute_data = abs(static_cast<s64>(to_cast));
			absolute_data = llabs(to_cast);
			ret.set_sign(get_bits_with_range(static_cast<s64>(to_cast),
				(width - 1), (width - 1)));
		}
	...
	//inline CastToType cast_to_int(std::ostream& os) const
	{
		...
		static constexpr s32 max_shift_amount 
		...
		if (curr_exp < 0)
		{
			...
			//	curr_exp, ", ", 
			//	static_cast<s32>(width_of_type(ret)), ", ",
			//	enc_exp(), "\n");
		...
		else // if (curr_exp >= 0)
		{
				...
				if (sizeof(ret) > sizeof(data()))
				{
					if (sticky)
					{
						if (!cast_to_type_is_signed)
						{
							...
							if (std::is_same<u64, decltype(ret)>::value)
							{
								if (sign())
								{
									ret = 0;
			...
			else // if (abs_curr_exp > max_shift_amount)
			{
				...
				if constexpr (sizeof(ret) > sizeof(data()))
				{
					...
					if ((curr_exp >= static_cast<s32>(width_of_type(ret)))
						&& (enc_exp() != max_enc_exp))
					{
						if (!cast_to_type_is_signed)
						{
	...
	inline s16 oracle_to_s16() const
	{
		const float ret_float = static_cast<float>(*this);

		return static_cast<s16>(ret_float);
	}
	...
	inline s32 exponent() const
	{
		s32 ret = static_cast<s32>(enc_exp());
		ret -= modded_bias;
		//ret = static_cast<s8>(ret);

		return ret;
	}
	//inline s32 set_exponent(s32 n_exponent)
	//{
...
#endif		// src__slash__bfloat16_class_hpp
